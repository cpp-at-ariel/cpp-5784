...
struct ReporterEmpty: public IReporter {
    ...
    ReporterEmpty(const ContextOptions& in)
    ...
    void test_run_end(const TestRunStats& /*in*/) override {
        std::cout << "Empty reporter done!" << std::endl;
    }
    void test_case_start(const TestCaseData& in) override { tc = &in; }
    void test_case_reenter(const TestCaseData& /*in*/) override {} // called when a test case is reentered because of unfinished subcases
...
REGISTER_REPORTER("empty", 1, ReporterEmpty);
